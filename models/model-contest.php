...
/**
* Model class <i>cf_Contest</i> represents contest.
* @package chModel
*/
class cf_Contest
{
    ...
    */
    function __construct($data)
    {
        ...
        $this->data['ref_variable'] = apply_filters('contestfriend_ref_variable', 'ref', $this);
    }
    
    /**
    * Returns post ID of custom post type 'contestfriend' with 'auto-draft' status. If none exists, it creates new one.
    ...
    */
    public static function get_new_id()
    {
        ...
        $args = array('post_type' => cf_Manager::post_type, 'post_status' => 'auto-draft');
        ...
        if(count($query->posts)==0) // auto draft doesnt exist, create new
        {
            ...
            update_post_meta($new_id, 'cf_disclaimer_rules_type', 'none');
            update_post_meta($new_id, 'cf_media_description_layout', 'media-top');
            update_post_meta($new_id, 'cf_widget_size', '640');
            update_post_meta($new_id, 'cf_headline_color', '#ffffff');
            update_post_meta($new_id, 'cf_headline_font', 'arial');
            update_post_meta($new_id, 'cf_description_color', '#000000');
            update_post_meta($new_id, 'cf_description_font', 'arial');
            update_post_meta($new_id, 'cf_title_background_color', '#40b3df');
            update_post_meta($new_id, 'cf_background_color', '#ffffff');
            update_post_meta($new_id, 'cf_border_color', '#2a71a2');
            update_post_meta($new_id, 'cf_winners_num', '1');
            update_post_meta($new_id, 'cf_referral_entries', '1');
            update_post_meta($new_id, 'cf_timezone', $datetimes['timezone']);
            update_post_meta($new_id, 'cf_date_start', $datetimes['start']);
            update_post_meta($new_id, 'cf_date_end', $datetimes['end']);
            update_post_meta($new_id, 'cf_submit_text', __('Join sweepstakes', 'contestfriend'));
            update_post_meta($new_id, 'cf_from_email', '"contestfriend" <'.get_option('admin_email').'>');
    ...
    */
    protected function get($id)
    {
        ...
        $args = array('post__in' => array($id), 'post_type' => cf_Manager::post_type, 'post_status' => array('publish', 'auto-draft'));
        $query = new WP_Query($args);
        ...
        foreach($data as $entry)
        {
            ...
            if($entry['meta_key']=='cf_from_email')
    ...
    */
    function get_all_entries()
    {
        ...
        global $wpdb;
        $sql = 'SELECT id FROM `'.$wpdb->prefix.cf_Manager::sqlt_participant.'` WHERE `contest_id`='.esc_sql($this->data['ID']).' AND `status`!="not_valid" AND `status`!="not_confirmed"';
        $data = $wpdb->get_results($sql, ARRAY_A);
        ...
        $sql = 'SELECT COUNT(*) FROM `'.$wpdb->prefix.cf_Manager::sqlt_participant_meta.'` WHERE `meta_key`="referral_to" AND `meta_value` IN ('.$in_qry.')';
        $num = $wpdb->get_var($sql);
        if($num!=NULL)
            $num_referrals = $num;
        
        return intval($num_participants)+(intval($num_referrals)*intval($this->data['cf_referral_entries']));   
    }
    ...
    */
    ...
    /**
    * Runs customizable SELECT database query and returns array of <i>cf_Contest</i> objects.
    ...
    * @return mixed Array of cf_Contest objects or empty array.
    */
    static function get_all($in_orderby = '', $in_order = '', $in_page = '', $in_perpage = '', $in_status = '')
    {
        ...
        if($in_orderby=='title')
        {
            ...
            $orderby_where = ' AND t3.meta_key="cf_headline"';
        }
        else if($in_orderby=='participants')
        {
            $sql_tbl = $wpdb->prefix.cf_Manager::sqlt_participant;
        ...
        else if($in_orderby=='date_start')
        {
            $orderby = ' ORDER BY t3.`meta_value`';
            $orderby_join = ' LEFT JOIN `'.$wpdb->postmeta.'` AS t3 ON t1.ID=t3.post_id';
            $orderby_where = ' AND t3.meta_key="cf_date_start"';
        }
        else if($in_orderby=='status')
        {
            $orderby = ' ORDER BY t3.`meta_value`';
            $orderby_join = ' LEFT JOIN `'.$wpdb->postmeta.'` AS t3 ON t1.ID=t3.post_id';
            $orderby_where = ' AND t3.meta_key="cf_status"';
        }
        else
        ...
        $sql = 'SELECT t1.ID'.$orderby_select.' FROM '.$wpdb->posts.' AS t1 LEFT JOIN '.$wpdb->postmeta.' AS t2 ON t1.ID=t2.post_id'.$orderby_join.' WHERE t1.`post_type`="contestfriend" AND t2.`meta_key`="cf_status"'.$status.$orderby_where.$filter.$orderby_groupby.$orderby.$order.$limit;
        $data = $wpdb->get_results($sql, ARRAY_A);
        ...
        {
            $contest = new cf_Contest($post['ID']);
            if($contest->_valid)
                $result[] = $contest;       
        }
    ...
    */
    ...
    {
        global $wpdb;
        $sql = 'SELECT count(ID) FROM '.$wpdb->posts.' WHERE post_type="contestfriend" AND post_status!="auto-draft"';
        $num = $wpdb->get_var($sql);
        if($num==NULL)
            return 0;
        return $num;
    ...
    /**
    * Creates, loads and returns <i>cf_Contest</i> object that was referred by specified referral code.
    * @param $code Referral code.
    * @return cf_Contest
    */
    public static function get_from_code($code)
    {
        $tbl_participant = $wpdb->prefix.cf_Manager::sqlt_participant;
    ...
    */
    function save()
    {
        ...
        update_post_meta($this->data['ID'], 'cf_status', 'active');
    ...
    */
    ...
    {            
        global $wpdb;                  
        update_post_meta($this->data['ID'], 'cf_status', esc_sql($status));
    }
    ...
    */
    ...
    {
        $real_time = strtotime($this->data['cf_date_end']);
        if($what=='start')
            $real_time = strtotime($this->data['cf_date_start']);
        
        $timezone = $this->data['cf_timezone'];
        if(!empty($timezone))
        {
    ...
    */
    function is_expired()
    {
        if($this->data['cf_status']=='expired' || $this->data['cf_status']=='winners_picked' || $this->data['cf_status']=='expired_auto')
            return true;
    ...
    */
    ...
    {        
        ...
        $participants = cf_Participant::get_all($this->data['ID'], '', '' ,'', '', 'all');
